...
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps complets (identité, profil, expériences, formation, compétences/langues, complémentaires). Liens CSS/JS versionnés `?v=20260426-ux2` (css) / `?v=20260428-A` (js). Layout 2 colonnes `#form-panel` (45%) + `#preview-panel` sticky (55%). Step 5 : `.tags-input-wrap` compétences + `#suggestions-competences`. Step 6 : certifications (`data-field="certifications"` + `#suggestions-certifications`, oninput avec `buildExpContext()`), intérêts (`data-field="interets"` + `#suggestions-interets`, oninput avec `buildExpContext()`), complément (`data-field="complement"` + `#suggestions-infos_complementaires`) — chaque champ déclenche `triggerSuggestions` en oninput. Shell branché sur `lang.js?v=20260426-cvf-i18n2` + `cv-form.js?v=20260428-A`. Bouton langue auto dans nav desktop/mobile. |
