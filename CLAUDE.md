...
| `api/create-checkout.js` | Création session Stripe Checkout — accepte paramètre optionnel `returnPath` (validé contre allowlist `['/service', '/cv-form']`, défaut `/service`) pour construire l'URL de retour Stripe |
...
| `api/suggest.js` | Suggestions IA temps réel pour champs CV — POST `{ field, poste, context? }`, fields acceptés : `accroche / missions / formation / competences / interets / certifications / infos_complementaires`, normalisation `missions_N → missions` et `formation_N → formation` côté serveur, rate-limit 10/min/IP, modèle Haiku, max_tokens 800, retourne `{ suggestions: string[] }`. Volumes : 6 accroches, 8 missions, 6 formations (diplômes/BTS/Licence/Master), 12 compétences, 8 intérêts (contextualisés avec `context`), 8 certifications (contextualisées avec `context`), 5 infos. Prompts retournent un tableau JSON direct. Nettoyage markdown avant `JSON.parse`. |
...
| `cv-wizard.html` | Tunnel CV Page 1 — galerie de 12 templates avec filtres par catégorie, miniatures inline HTML/CSS scalées (CV Yonel GOVINDIN, masculin, Assistant Administratif Polyvalent, 5 ans exp., 3 postes dont CNAF, 5 compétences, 4 langues dont Portugais), modale plein écran + module toggle photo BEM. Le shell visible est branché sur `lang.js` (`lang.css` + `lang.js?v=20260425-cvw-i18n`) : nav, hero, filtres, modale, étape 2, footer. Le bouton de choix de langue est injecté automatiquement dans la navbar desktop/mobile. CTA modale `#btn-choose` avec `data-i18n="cvw_choose_cta"` + `onclick` inline : lit `cv_template` + `cv_with_photo` depuis sessionStorage, redirige vers `/cv-wizard?step=2&template=NN`. Écran step=2 intégré (id `step2-screen`) avec : confirmation template (icône animée, nom, badge photo), séparateur doré, 3 cards mode (scratch/improve/target via `onclick` inline — `this.dataset.mode` → `sessionStorage.cv_mode`), `#btn-step2-continue` (désactivé par défaut `pointer-events:none;opacity:0.4`, activé au choix de mode via `.enabled`, redirige → `/cv-form`), `#btn-step2-back` (→ `/cv-wizard.html`). Chaque zone photo template porte la classe `cv-photo-zone`. Liens CSS/JS versionnés `?v=20260429-A2`. Point d'entrée depuis `index.html` CTA CV. |
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps complets (identité, profil, expériences, formation, compétences/langues, complémentaires). Liens CSS/JS versionnés `?v=20260429-A4` (css) / `?v=20260429-A4` (js). En-tête `#form-panel` : bouton « ↻ Recommencer » (`.btn-reset-cv` + `onclick="if(confirm(...))resetState()"`). Layout 2 colonnes `#form-panel` (45%) + `#preview-panel` sticky (55%). Step 4 : `#suggestions-formation-global` (zone centralisée de suggestions de diplômes, au-dessus de `#formations-container`). Step 5 : `.tags-input-wrap` compétences + `#suggestions-competences`. Step 6 : certifications (`data-field="certifications"` + `#suggestions-certifications`, oninput avec `buildExpContext()`), intérêts (`data-field="interets"` + `#suggestions-interets`, oninput avec `buildExpContext()`), complément (`data-field="complement"` + `#suggestions-infos_complementaires`) — chaque champ déclenche `triggerSuggestions` en oninput. Step 1 : champ prénom auto-capitalise (1re lettre maj + reste min), champ nom force majuscules (oninput) ; email avec `pattern="^[^@]+@[^@]+\.[^@]+$"` ; téléphone avec `pattern="^(\+594|0)\s?[1-9](\s?\d{2}){4}$"` (Guyane) ; code postal avec `pattern="^973[0-9]{2}$"` + `maxlength="5"`. Shell branché sur `lang.js?v=20260426-cvf-i18n2` + `cv-form.js?v=20260429-A4`. Bouton langue auto dans nav desktop/mobile. |
| `cv-form.css` | Styles Page 3 — `.suggestions-wrap` en `flex-direction:column` (chips en lignes complètes, pas horizontaux). `.suggestion-chip` : animation `@keyframes chipEnter` (fade + slide-up 0.22s spring, délai décalé par index via `animation-delay` inline), `white-space:normal`, `word-break:break-word`, `border-radius:10px`. `.suggestion-skeleton` : placeholder shimmer 3 blocs (`@keyframes shimmerChip` 1.4s) affiché pendant le chargement IA. Navbar dark premium, layout grid 2 colonnes, progress bar or, champs, tags, recap. `#preview-panel` : `overflow-y:auto; overflow-x:hidden` (scrollable) + scrollbar or personnalisée ; `#preview-container` : `height:auto; align-items:flex-start` (contenu haut, pas centré verticalement). Bloc post-génération : `#cv-loading-overlay` (overlay sombre, spinner or 64px, animation `cvSpin`, étapes 1/4 → 4/4), `.cv-watermarked::after` (filigrane « DOK'PÉYI · APERÇU » 64px or transparent rotation -30°), `#cv-final-screen` (modale dark luxe, CTAs « Finaliser » / « Modifier »), `.btn-reset-cv` (bouton outline subtil, hover or), `#cv-success-screen` (overlay plein-écran fond sombre, `.cv-success-content` fond vert nuit, `.cv-success-icon` cercle vert animé `@keyframes successPop`). |
| `cv-form.js` | Logique Page 3 — helper `escapeHtml(s)` centralisé (5 substitutions HTML). Persistance localStorage TTL 24h via `STORAGE_KEY='dokpeyi_cv_state'` + `saveState()` (try/catch silencieux, exclut `photo` et `cvData.photo` racine) + `loadState()` (TTL check) + `resetState()` (clear + `location.reload()`). `saveState()` est appelé dans `updateCvData`, `updateExp`, `updateFormation`, `updateLangue`, `addTag`, `removeTag`, `removeCard` et `handlePdfUpload`. Au DOMContentLoaded : `loadState()` → `Object.assign(cvData, saved)` + `delete cvData.ts` puis `rebuildAllCardsFromData()` ; sinon comportement par défaut (1 exp + 1 form + 1 langue vides). `submitForm()` : `validateStep(6)` → `showLoadingScreen()` (overlay #cv-loading-overlay + setInterval 2.5s défilement étapes 1/4→4/4) → POST `/api/orchestrate` `{type:'cv',data:cvData,prompt,template,withPhoto,mode}` → `hideLoadingScreen()` + `showFinalPreview(result)` (applique `result.optimized.{accroche,experiences,competences}`, ajoute `.cv-watermarked` sur `#preview-panel`, ouvre #cv-final-screen avec CTAs `goToCheckout` / `closeFinalScreen`). `buildOrchestratePrompt(d)` agrège identité/profil/exp/formations/compétences/langues/extras + offre ciblée (mode target) + consigne IA. `handlePdfUpload` réécrit en JSON via `fileToBase64(file)` Promise → POST `/api/extract-doc` `{file:{data:base64,type}}` → fusionne champs (`prenom/nom/email/phone|tel/poste/accroche/experiences/formations/competences/langues`), reset `lastPoste`, appelle `rebuildAllCardsFromData()` + `updatePreview()` + `saveState()`. `rebuildAllCardsFromData()` : vide les cards `[id^="exp-card-"]/[id^="form-card-"]/[id^="lang-card-"]`, reset compteurs, ré-appelle `addExperience/addFormation/addLangue` puis remplit via `card.querySelectorAll('input.field-input')` + `textarea.field-textarea` + `select.field-select` ; remplit identité/profil/extras via `[data-field="..."]` (mapping spécifique `extras.infos → data-field="complement"`). `renderTemplate` applique `escapeHtml` sur toutes les données utilisateur (identité, profil, expériences, formations, compétences, langues) ; missions affichées en `<ul><li>` échappés ; extras (`certifications`, `interets`, `infos`) rendus en bas du CV preview via `buildLayout` (chips pour certif/intérêts, paragraphe pour infos). `renderTags` utilise `escapeHtml` + `data-value`/`data-section` sur le bouton remove (plus d'injection JS via onclick). Chips suggestions (`triggerSuggestions`, `initCompSuggestions`) : `escapeHtml` + `data-value` + `animation-delay` décalé (×40ms par index) ; pendant le chargement : 3 `.suggestion-skeleton` au lieu du texte (aria-label contient la clé i18n `cvf_suggestions_loading`) ; debounce `DEBOUNCE_MS = 300` ms. Chips compétences (liste statique `initCompSuggestions`) : onclick appelle `addTag` + `this.parentNode.setAttribute('data-frozen','true')` + `this.remove()` — comportement cohérent avec missions/formation. `applySuggestion` reçoit la valeur depuis `this.dataset.value` ; cas `formation-global` : remplit le premier `form-card-1 input` vide ou crée une nouvelle card via `addFormation()`. `addFormation()` ne génère plus de conteneur `suggestions-formation_N` par card (suggestions centralisées dans `#suggestions-formation-global`) ; le champ diplôme n'appelle plus `triggerSuggestions` en oninput. `validateStep(step)` : valide identité (step 1 — prenom/nom/email/tel + format email), profil (step 2 — poste/accroche), expériences (step 3), formations (step 4), compétences ≥ 3 (step 5). `nextStep()` appelle `validateStep` et bloque si erreurs (`alert`). `updateExp`, `updateFormation` et `updateLangue` appellent `updatePreview()` en fin de fonction. `addLangue()` appelle `updatePreview()` après ajout. `removeCard()` : filtre les nulls de `cvData[section]`, renumérote les cards restantes (IDs réels : `exp-card-`, `form-card-`, `lang-card-` ; titles via `data-cvf-card-title`), décrémente les compteurs, appelle `updatePreview()` + `saveState()`. `triggerSuggestions` : freeze (chips + loading) pour les champs standard — `missions_N` et `formation-global` utilisent `data-frozen` (gel manuel après sélection chip, reset via `removeAttribute` quand le poste est vidé) ; chip missions/formation set `data-frozen` sur `parentNode` AVANT `applySuggestion` pour éviter re-trigger synchrone ; changement de poste (isPosteField = accroche|competences|interets + poste !== lastPoste) vide ciblés `['accroche','competences','interets','certifications','infos_complementaires']` + tous `[id^="suggestions-formation_"]` + `#suggestions-formation-global` (reset `innerHTML` + `data-frozen`) ; `lastPoste` initialisé au DOMContentLoaded depuis `cvData.profil.poste`. `cvData.withPhoto` re-lu depuis sessionStorage au DOMContentLoaded (source de vérité unique, jamais réécrit dans cv-form.js). Trigger automatique : step 4 → `formation-global`, step 5 → `competences` (avec `buildExpContext()`), step 6 → `certifications` + `interets` (avec `buildExpContext()`). `buildExpContext()` : retourne les postes des expériences non-null joinées par `, `. `updateRecap()` : inclut extras (certifications, intérêts, infos) depuis `cvData.extras`. `CV_TUNNEL_PRICES` : supplément par template 01-12 (`{01:0,02:2,...,12:7}`), `CV_BASE_PRICE=8`. `goToCheckout()` : calcule montant (base 8€ + supplément template), génère `orderId='cv_'+Date.now()+'_'+random`, stocke `{id,service,montant,cvHtml,prenom,nom,email,poste}` dans `sessionStorage['dokpeyi_cv_pending_'+orderId]`, désactive le bouton, POST `/api/create-checkout` `{service:'cv',amount,orderId,email,nom,prenom,returnPath:'/cv-form'}`, redirige vers `data.url` (Stripe) ou restaure le bouton sur erreur. `showSuccessScreen(order)` : overlay plein-écran dark green, icône animée ✓, titre/corps i18n, email destinataire, référence commande, CTA retour accueil. Au DOMContentLoaded : vérifie `?success=1&order_id=...` en premier — si présent, lit et efface `sessionStorage['dokpeyi_cv_pending_'+orderId]`, appelle `showSuccessScreen()`, lance en fire-and-forget les emails `order_confirmation` + `new_order_admin` via `/api/send-email`, puis `return` (bypass de l'init du formulaire). Branché sur `DokPeyiI18n` + `dokpeyi:langchange`. Globals exposés : `nextStep`, `prevStep`, `validateStep`, `submitForm`, `toggleMobilePreview`, `updateCvData`, `updateExp`, `removeCard`, `addExperience`, `handlePdfUpload`, `handlePhotoUpload`, `triggerSuggestions`, `applySuggestion`, `updatePreview`, `addFormation`, `updateFormation`, `addLangue`, `updateLangue`, `handleTagInput`, `addTag`, `removeTag`, `cvData`, `buildExpContext`, `resetState`, `goToCheckout`, `closeFinalScreen`. |
| `cv-wizard.css` | Styles dédiés au tunnel CV — navbar dark premium, filtre catégorie, grille 3-col responsive, cards templates, badges prix/populaire, modale overlay, module `.photo-selector` BEM, écran step=2 (`.step2-inner`, `.step2-confirm` flex-column + gap 12px, `.step2-check-icon` animation `checkPop`, `.step2-divider` flex + `::before/::after`, `.step2-modes` grid 3-col, `.mode-card` + `.mode-card.active` + `.mode-card-active`, `.mode-card__icon/__title/__desc/__badge` BEM, `#btn-step2-continue` base `pointer-events:none;cursor:not-allowed;opacity:0.4` / `.enabled` `pointer-events:auto;cursor:pointer;opacity:1`, `#btn-step2-back`), animations. Versionné `?v=20260429-A2`. |
| `cv-wizard.js` | Logique tunnel CV — un seul `DOMContentLoaded` fusionné. Détection step=2 via `URLSearchParams` : masque `.gallery-hero, .filter-section, .tpl-section`, affiche `#step2-screen`, remplit `#step2-template-name` (format `NN — Nom`) et `#step2-photo-badge`. Au step=2 : `sessionStorage.removeItem('cv_mode')` (purge la valeur stale) + override `#btn-step2-continue.onclick` avec guard `alert` si aucun mode sélectionné. `tplId` résolu depuis `params.get('template')` ou `sessionStorage.cv_template`. `openModal()` persiste immédiatement `cv_template` et initialise `cv_with_photo` dans sessionStorage. Pas de listener JS sur `#btn-choose` — remplacé par `onclick` inline dans le HTML. Tunnel CV branché sur `window.DokPeyiI18n` via `CVW_TEMPLATES` (`cvw_tpl_01_*` → `cvw_tpl_12_*`) ; écoute `dokpeyi:langchange` pour rerendre noms, catégories et descriptions des 12 templates, la modale ouverte et l'écran step=2 sans rechargement. Toggle photo par `onclick` inline sur `.photo-card`. sessionStorage : `cv_template`, `cv_with_photo`, `cv_mode`. Versionné `?v=20260429-A2`. |
...
- **Tests obligatoires avant push** : `node --test tests/*.test.js` (231 / 231 OK)
