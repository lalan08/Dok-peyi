...
- Tests : `node --test` natif (pas de framework externe), 226 tests / 72 suites
...
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps complets (identité, profil, expériences, formation, compétences/langues, complémentaires). Layout 2 colonnes : `#form-panel` (45%) + `#preview-panel` sticky (55%). Progress bar dorée `#progress-bar / #progress-fill`. Navigation `#btn-prev / #btn-next / #btn-submit`. Overlay aperçu mobile via `#btn-preview-mobile` + `#btn-close-preview`. Watermark aperçu `#preview-watermark`. Reçoit `cv_template`, `cv_with_photo`, `cv_mode` depuis sessionStorage. `noindex`. Le shell visible est maintenant branché sur `lang.js` (`lang.css` + `lang.js?v=20260426-cvf-i18n`) : progression, 6 titres/sous-titres, labels statiques, placeholders, boutons, récapitulatif statique, watermark et contrôles d’aperçu. Le bouton de choix de langue est injecté automatiquement dans la nav desktop/mobile. Step 4 : `#formations-container` + btn-add-card. Step 5 : `.tags-input-wrap` compétences (#competences-wrap/tags/input) + suggestions prédéfinies (#suggestions-competences) + `#langues-container` + btn-add-card. Step 6 : certifications, intérêts, complément textareas + `#recap-body` récapitulatif. |
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps complets (identité, profil, expériences, formation, compétences/langues, complémentaires). Layout 2 colonnes : `#form-panel` (45%) + `#preview-panel` sticky (55%). Progress bar dorée `#progress-bar / #progress-fill`. Navigation `#btn-prev / #btn-next / #btn-submit`. Overlay aperçu mobile via `#btn-preview-mobile` + `#btn-close-preview`. Watermark aperçu `#preview-watermark`. Reçoit `cv_template`, `cv_with_photo`, `cv_mode` depuis sessionStorage. `noindex`. Liens CSS `?v=20260426-suggest`. Le shell visible est maintenant branché sur `lang.js` (`lang.css` + `lang.js?v=20260426-cvf-i18n2`) et `cv-form.js?v=20260426-cvf-i18n2` : progression, 6 titres/sous-titres, labels statiques, placeholders, boutons, récapitulatif statique, watermark et contrôles d’aperçu. Le bouton de choix de langue est injecté automatiquement dans la nav desktop/mobile. Step 4 : `#formations-container` + btn-add-card. Step 5 : `.tags-input-wrap` compétences (#competences-wrap/tags/input) + suggestions prédéfinies (#suggestions-competences) + `#langues-container` + btn-add-card. Step 6 : certifications, intérêts, complément textareas + `#recap-body` récapitulatif. |
| `cv-form.css` | Styles Page 3 — variables identiques à cv-wizard.css, navbar dark premium complète, layout grid 2 colonnes `45fr 55fr`, `#form-panel` scrollable, `#preview-panel` sticky, progress bar gradient or, `.step-title / .step-subtitle`, `#form-nav` flex, `.field-group / .field-label / .field-input / .field-textarea / .field-row / .field-select / .field-hint`, `.suggestion-chip`, `.ai-info-badge`, `.dynamic-card / .btn-remove-card / .btn-add-card`, `.photo-upload-zone`, `.pdf-upload-zone`, `.tags-input-wrap / .tag-chip / .tag-remove / .tags-input`, `.recap-box / .recap-title / .recap-body`, overlay `.preview-overlay-open`, responsive 768px/480px. |
| `cv-form.js` | Logique Page 3 — IIFE strict. `cvData` étendu (photo, identite, profil, experiences, formations, competences, langues, extras). `updateCvData`, `addExperience`, `updateExp`, `removeCard` (générique). `addFormation`, `updateFormation` : cards dynamiques diplôme/établissement/année/mention. `handleTagInput`, `addTag`, `removeTag`, `renderTags` : gestion tags compétences (Enter ou virgule). `addLangue`, `updateLangue` : cards dynamiques langue/niveau (Notions → Langue maternelle). `updateRecap` : récap step 6 (nom, poste, nb exp/form, compétences, langues). `handlePdfUpload`, `handlePhotoUpload`. `triggerSuggestions(field, poste, context)` : async + debounce 600ms, appelle `/api/suggest`, affiche `.suggestions-loading` pendant le chargement, injecte les chips. `applySuggestion(field, value)` : dispatche selon field (accroche→textarea, missions_N→textarea exp, competences→tag push, interets→input). `getTemplateStyles(tpl)`, `buildLayout(tpl, s, d)`, `renderTemplate(cvData)`. `updatePreview()` : cssText forcé + `renderTemplate` + `requestAnimationFrame` scale. `showStep(n)` : déclenche `triggerSuggestions('competences',...)` à l'étape 5. Le module est maintenant branché sur `window.DokPeyiI18n` pour le formulaire CV actif : helper `cvfT`, états suggestions, états import PDF, labels/placeholders dynamiques des cards expériences / formations / langues, récapitulatif, options de selects et rerender à chaud sur `dokpeyi:langchange`. DOMContentLoaded : ordre strict — `showStep(1)` → modes/photos → `addExperience()` → `addFormation()` → `addLangue()` → `updatePreview()` → `applyCvFormDynamicTranslations()` en dernier. Expose tous les helpers + `cvData` sur `window`. |
...
- **Tests obligatoires avant push** : `node --test tests/*.test.js` (226 / 226 OK)
...
- **Résultat actuel** : 226 tests / 72 suites / 226 pass / 0 fail
- **Couverture** :
