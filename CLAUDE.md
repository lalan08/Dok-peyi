...
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps (identité, profil, expériences, formation, compétences/langues, complémentaires). Layout 2 colonnes : `#form-panel` (45%) + `#preview-panel` sticky (55%). Progress bar dorée `#progress-bar / #progress-fill`. Navigation `#btn-prev / #btn-next / #btn-submit`. Overlay aperçu mobile via `#btn-preview-mobile` + `#btn-close-preview`. Watermark aperçu `#preview-watermark`. Reçoit `cv_template`, `cv_with_photo`, `cv_mode` depuis sessionStorage. `noindex`. Liens CSS `?v=20260425-s3a`. |
| `cv-form.css` | Styles Page 3 — variables identiques à cv-wizard.css, navbar dark premium complète, layout grid 2 colonnes `45fr 55fr`, `#form-panel` scrollable, `#preview-panel` sticky, progress bar gradient or, `.step-title / .step-subtitle`, `#form-nav` flex, `.field-group / .field-label / .field-input / .field-textarea / .field-row`, `.suggestion-chip`, `.ai-info-badge`, `.dynamic-card / .btn-remove-card / .btn-add-card`, `.photo-upload-zone`, `.pdf-upload-zone`, overlay `.preview-overlay-open`, responsive 768px/480px. |
| `cv-form.js` | Logique Page 3 — IIFE strict. `cvData` étendu (photo, identite, profil, experiences, formations, compétences, langues, extras). `updateCvData(section, field, value)`. `addExperience()` : injecte une `.dynamic-card` numérotée avec champs poste/entreprise/dates/missions. `updateExp(n, field, value)`, `removeCard(cardId, section, n)`. `handlePdfUpload(file)` : appel `/api/extract-doc`, pré-remplit cvData. `handlePhotoUpload(input)` : FileReader → cvData.photo + préview. `triggerSuggestions(field, poste)` : suggestions statiques par défaut (S3-D : IA). `applySuggestion(field, value)`. `updatePreview()` : préview nom+poste (S3-C : template complet). DOMContentLoaded : `showStep(1)`, masque photo si `cv_with_photo=false`, affiche offre-cible si mode=target, affiche pdf-upload si mode=improve sinon `addExperience()`. Expose tous les helpers sur `window`. |
...
- **Phase 3 — Premium et croissance** : en cours — Tunnel CV Page 1 (galerie 12 templates) ✅ livré. Le shell visible du tunnel CV est maintenant branché sur l'i18n centralisée (`lang.js`) avec sélecteur de langue dans la nav et rerender dynamique des cartes / modale / étape 2. Le catalogue 6 templates `cv-catalogue.html` est aussi branché sur l'i18n centralisée avec sélecteur de langue et rerender dynamique des cartes. Les pages `404.html`, `mentions-legales.html`, `cgv.html` et `confidentialite.html` sont maintenant branchées sur l'i18n centralisée avec topbar légère et sélecteur de langue. Pages 2–4 (infos, paiement, confirmation) : à venir. Fondation i18n JS+DOM centralisée posée dans `lang.js`. `index.html`, `a-propos.html`, `service.html` (shell), `cv-catalogue.html`, `cv-wizard.html` (shell + tunnel dynamique), `404.html`, `mentions-legales.html`, `cgv.html` et `confidentialite.html` sont maintenant branchés pour les textes visibles ; reste à couvrir le reste de `service.js` et les autres pages publiques.
