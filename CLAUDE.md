...
| `cv-wizard.js` | Logique tunnel CV — routing step=2 (URLSearchParams → écran confirmation local, zéro 404), calcul dynamique scale previews, filtre catégorie, modale (open/close/Escape), `detectPhotos()` (cible `.cv-photo-zone`), toggle photo via `document.addEventListener('click')` event delegation (`.photo-card[data-value]` → active class + `#modal-preview-inner .cv-photo-zone` max-height/opacity), sessionStorage `cv_template` + `cv_with_photo`. |
