...
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps complets (identité, profil, expériences, formation, compétences/langues, complémentaires). Liens CSS/JS versionnés `?v=20260426-ux2`. Layout 2 colonnes `#form-panel` (45%) + `#preview-panel` sticky (55%). Step 5 : `.tags-input-wrap` compétences + `#suggestions-competences`. Step 6 : certifications (`data-field="certifications"` + `#suggestions-certifications`), intérêts (`data-field="interets"` + `#suggestions-interets`), complément (`data-field="complement"` + `#suggestions-infos_complementaires`) — chaque champ déclenche `triggerSuggestions` en oninput. Shell branché sur `lang.js?v=20260426-cvf-i18n2` + `cv-form.js?v=20260426-ux2`. Bouton langue auto dans nav desktop/mobile. |
...
| `cv-form.js` | Logique Page 3 — `applySuggestion(field, value)` : accroche additif (`current + ‘ ‘ + value`), missions additif (`\n• value`), competences push, interets additif (`, value`), certifications additif (`, value`), infos_complementaires additif (`\n value`). Trigger missions sur intitulé poste de la card expérience avec `this.value` (non `cvData.profil.poste`). `triggerSuggestions` envoie `missions_N` au serveur (normalisé côté API). Branché sur `DokPeyiI18n` + `dokpeyi:langchange`. |
