...
| `api/suggest.js` | Suggestions IA temps réel pour champs CV — POST `{ field, poste, context? }`, fields acceptés : `accroche / missions / competences / interets / certifications / infos_complementaires`, normalisation `missions_N → missions` côté serveur, rate-limit 10/min/IP, modèle Haiku, max_tokens 300, retourne `{ suggestions: string[] }`. Volumes : 6 accroches, 8 missions, 12 compétences, 8 intérêts, 6 certifications, 5 infos. Prompts retournent un tableau JSON direct. Nettoyage markdown avant `JSON.parse`. |
...
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps complets (identité, profil, expériences, formation, compétences/langues, complémentaires). Liens CSS/JS versionnés `?v=20260426-ux2` (css) / `?v=20260426-ux3` (js). Layout 2 colonnes `#form-panel` (45%) + `#preview-panel` sticky (55%). Step 5 : `.tags-input-wrap` compétences + `#suggestions-competences`. Step 6 : certifications (`data-field="certifications"` + `#suggestions-certifications`), intérêts (`data-field="interets"` + `#suggestions-interets`), complément (`data-field="complement"` + `#suggestions-infos_complementaires`) — chaque champ déclenche `triggerSuggestions` en oninput. Shell branché sur `lang.js?v=20260426-cvf-i18n2` + `cv-form.js?v=20260426-ux3`. Bouton langue auto dans nav desktop/mobile. |
...
| `cv-form.js` | Logique Page 3 — `applySuggestion(field, value)` : accroche additif (`\n` sep), missions additif (`\n•`), competences push, interets/certifications additif (`, `), infos_complementaires additif (`\n`). `triggerSuggestions` : freeze si chips déjà affichées ou loading en cours ; changement de poste (field=accroche + poste !== lastPoste) vide toutes les `.suggestions-wrap` ; chip cliquée supprimée via `this.remove()` ; `lastPoste` tracke le dernier poste connu. Trigger missions sur intitulé poste de la card expérience avec `this.value`. `triggerSuggestions` envoie `missions_N` au serveur (normalisé côté API). Branché sur `DokPeyiI18n` + `dokpeyi:langchange`. |
