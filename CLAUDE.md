...
| `cv-wizard.js` | Logique tunnel CV — routing step=2 (URLSearchParams → `showStep2Screen()` hoistée), redirections en chemins absolus (`/cv-wizard.html?step=2`), calcul dynamique scale previews, filtre catégorie, modale (open/close/Escape), `openModal()` reset photo toggle. Toggle photo par `onclick` inline sur `.photo-card`. `showStep2Screen()` : map TPL_NAMES (12 templates), remplit `#step2-template-name` et `#step2-photo-badge`, masque galerie, affiche `#step2-screen`. sessionStorage : `cv_template`, `cv_with_photo`, `cv_mode`. |
