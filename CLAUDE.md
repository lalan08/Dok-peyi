...
| `cv-form.html` | Tunnel CV Page 3 — formulaire multi-étapes 6 steps complets (identité, profil, expériences, formation, compétences/langues, complémentaires). Liens CSS/JS versionnés `?v=20260426-ux2` (css) / `?v=20260427-fixes2` (js). Layout 2 colonnes `#form-panel` (45%) + `#preview-panel` sticky (55%). Step 5 : `.tags-input-wrap` compétences + `#suggestions-competences`. Step 6 : certifications (`data-field="certifications"` + `#suggestions-certifications`, oninput avec `buildExpContext()`), intérêts (`data-field="interets"` + `#suggestions-interets`, oninput avec `buildExpContext()`), complément (`data-field="complement"` + `#suggestions-infos_complementaires`) — chaque champ déclenche `triggerSuggestions` en oninput. Shell branché sur `lang.js?v=20260426-cvf-i18n2` + `cv-form.js?v=20260427-fixes2`. Bouton langue auto dans nav desktop/mobile. |
...
| `cv-form.js` | Logique Page 3 — `applySuggestion(field, value)` : accroche additif (`\n` sep), missions additif (`\n•`), formation_N additif (remplir diplome si vide, sinon créer nouvelle card via `addFormation()`), competences push, interets/certifications additif (`, `), infos_complementaires additif (`\n`). `updateExp` et `updateFormation` appellent `updatePreview()` en fin de fonction. `triggerSuggestions` : freeze (chips + loading) pour les champs standard — `missions_N` et `formation_N` utilisent `data-frozen` (gel manuel après sélection chip, reset via `removeAttribute` quand le poste/diplome est vidé) ; chip missions/formation set `data-frozen` sur `parentNode` AVANT `applySuggestion` pour éviter re-trigger synchrone ; changement de poste (isPosteField = accroche|competences|interets + poste !== lastPoste) vide ciblés `['accroche','competences','interets','certifications','infos_complementaires']` ; `lastPoste` initialisé au DOMContentLoaded depuis `cvData.profil.poste`. `cvData.withPhoto` re-lu depuis sessionStorage au DOMContentLoaded (source de vérité unique, jamais réécrit dans cv-form.js). Trigger automatique : step 4 → `formation_1`, step 5 → `competences`, step 6 → `certifications` + `interets` (avec `buildExpContext()`). `buildExpContext()` : retourne les postes des expériences non-null joinées par `, `. Preview accroche : HTML-escapée + `\n→<br>` (via `accrocheHtml` dans `renderTemplate`). Branché sur `DokPeyiI18n` + `dokpeyi:langchange`. |
